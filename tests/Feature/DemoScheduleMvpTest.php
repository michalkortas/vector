...
it('loads demo image data and per resource limits from json source', function (): void {
    ...
    expect(DB::table('resources')->count())->toBe(24)
        ...
        ->and((int) DB::table('resources')->where('id', $resource5Id)->value('nominal_workday_minutes'))->toBe(455)
        ->and((int) DB::table('absence_types')->where('code', 'vacation')->value('nominal_minutes_per_day'))->toBe(455)
        ->and((int) $resource5VacationMinutes)->toBe(4550)
        ->and((int) DB::table('absences')->where('resource_id', $resource12Id)->sum('nominal_minutes'))->toBe(3640)
